  ...
  $(this).off('mousewheel.disableScroll')
    ...
    //  */
     ...
     $(".addmore").on('click',function(){
        ...
        html ='<tr>';
        html += '<td><select name="fk_sub_category_id[]" data-placeholder="- Select -" class="select form-control select" tabindex="10" required="required"><option value="">Please choose</option><?php foreach ($subCategories as $subCategory){ ?><option value="<?php echo $subCategory->id; ?>"><?php echo $subCategory->sub_category_name; ?></option><?php } ?></select></td>';
        html+='<td></td>';
    ...
    $.ajax({
        ...
        success: function(result){
            if(result.asset_type===1){
                ...
                $('#loanId_1').load('<?php echo e(URL::to("receivable-loan-id")); ?>?bank='+bank);
                $('.addmore').fadeOut();
...
$(document).on('change','#loan_id_list',function(){
    ...
    if(id.length>0){
        $.ajax({
            url:path+'/receivable-loan-check/'+id,
            type:"GET",
            success:function(result){
                if(result[0]!=''){
                    if(result[1]==null){
